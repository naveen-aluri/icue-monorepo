...
class AddTyresExpensePage extends StatefulWidget {
  const AddTyresExpensePage({
    super.key,
  ...
  @override
  State<AddTyresExpensePage> createState() => _AddTyresExpensePageState();
}

class _AddTyresExpensePageState extends State<AddTyresExpensePage> {
  ...
  void initState() {
    ...
    getIt<AnalyticsService>().logScreenView(
      screenName: 'add-types-expense-page',
      parameters: {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Tyres Expense')),
      body: Form(
        key: _formKey,
            ...
            children: [
              Dropdown(
                ...
                items: getDropDownMenuItems(
                  null,
                  widget.expenseType.expenseTypeDetails
                      .singleWhere((e) => e.code == 'Type')
                      .details
                      ?.map((e) => MenuItem(id: e.name, name: e.name ?? ''))
                      .toList(),
                ),
              ),
              ...
              const SizedBox(height: 16),
              InputField(
                key: const Key('Tyre Name (Brand)'),
                label: 'Tyre Name (Brand)',
                initialValue: tyreBrand,
                isRequired: true,
                filled: false,
                onSaved: (val) => tyreBrand = val,
              ),
              const SizedBox(height: 16),
              InputField(
                key: const Key('Tyre Number/Size'),
                label: 'Tyre Number/Size',
                initialValue: tyreNoSize,
                isRequired: true,
                filled: false,
                onSaved: (val) => tyreNoSize = val,
              ),
              const SizedBox(height: 16),
              InputField(
              ...
              const SizedBox(height: 16),
              InputField(
                key: const Key('Warranty (In Months)'),
